...
- **기능 설명**: 설문조사 및 간단한 종합 퀴즈를 통해 여러 기술 스택의 **1차 전직(⭐)** 상태를 한 번에 부여하는 **실력 진단(Skill Calibration)** 기능.
...
### `3.2. 승급 심사 (Skill Advancement)`
- **기능 설명**: 각 기술 노드(예: Docker)의 숙련도를 1~3단계(전직)로 평가하는 **AI 면접 시스템**.
    - **1차 전직 (⭐)**: [개념 습득]: "이 기술이 무엇이고 언제 쓰는가?" (정의, 기초 문법, 사용 사례)
    - **2차 전직 (⭐⭐)**: [응용 능력]: "어떻게 잘 쓰는가?" (동작 원리, 트러블슈팅, 모범 사례)
    - **3차 전직 (⭐⭐⭐)**: [설계 및 최적화]: "왜 이렇게 설계했는가?" (내부 구조, 성능 튜닝, 트레이드오프 분석)
- **입력 데이터**: 도전하려는 전직 등급, 사용자 답변(텍스트/음성).
- **출력 결과**: 합격/불합격 여부, 획득한 별(Star), 피드백 리포트.
- **주요 로직 (Multi-Agent System)**:
    - **Interviewer Agent**: 페르소나가 부여된 면접관. 꼬리물기 질문을 생성하고 사용자의 답변을 유도.
    - **Evaluator Agent**: 답변의 정확성을 객관적으로 채점하고 JSON 포맷으로 평가 데이터 생성.
    - **Level-Adaptive Prompting**: 현재 도전 중인 등급(1~3차)에 맞춰 프롬프트의 난이도와 평가 기준(Rubric)을 동적으로 변경.
    - **Unified Feedback Report**: 면접 종료 후 합격/불합격 여부와 상관없이 **하나의 통합된 리포트**에서 결과, 별 획득 여부, 피드백을 한 번에 보여줌.
    - **One-Shot Evaluation**: AI가 대화 맥락을 분석하여 최종 평가를 JSON 포맷으로 생성.

### `3.3. 트랙 마스터 통합 퀴즈 (Track Master Challenge)`
- **기능 설명**: 특정 직무 트랙(예: AI Engineer 트랙)의 필수 노드를 일정 수준 이상 달성 시 도전 가능한 **최종 관문(Track Master Challenge)**.
...
- **주요 로직**:
    ...
    - **Retry Cooldown**: 불합격 시 재도전까지 일정 시간(Cool-time) 대기 필요.
